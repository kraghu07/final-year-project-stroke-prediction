...
def image():
    if request.method == 'POST':
        ...
        IMG_SIZE = 50
        LR = 1e-3
        MODEL_NAME = 'Strokepreiction-{}-{}.model'.format(LR, '2conv-basic')
    ##    MODEL_NAME='keras_model.h5'
        ...
        for num, data in enumerate(verify_data):
            ...
            if np.argmax(model_out) == 0:
                str_label = 'ischemic'
                print("The predicted image of the ischemic is with a accuracy of {} %".format(model_out[0]*100))
                accuracy="The predicted image of the ischemic is with a accuracy of {}%".format(model_out[0]*100)
                
            elif np.argmax(model_out) == 1:
                str_label = 'hemorrhagic'
                print("The predicted image of the hemorrhagic is with a accuracy of {} %".format(model_out[1]*100))
                accuracy="The predicted image of the hemorrhagic is with a accuracy of {}%".format(model_out[1]*100)
            ...
            dic={'Ischemic':A,'Hemorrhagic':B,'Normal':C}
            algm = list(dic.keys()) 
            ...
            plt.title("Accuracy Comparision between Brain Stroke prediction...")
